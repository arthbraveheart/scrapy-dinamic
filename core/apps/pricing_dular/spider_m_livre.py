...
search = load_pkl('dular_links').loc[:,['SKU','Cód PAI','M_Livre']]
...
# Cria o arquivo CSV
with open(out_path + f"Prices_M_Livre_{today}.csv", "w", newline="", encoding="utf-8-sig") as f:
    ...
    for i,row in search.iterrows():
        ...
        for name, price in zip(names,prices):
            if name!='empty':
                linha = [row['SKU'],row['Cód PAI'],name, price.replace('.',','), row['M_Livre']]
                print(linha)
